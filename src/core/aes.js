// AES constants
const STATE_SIZE = 16;      // Size of the state array in bytes (4x4 matrix)
const WORD_SIZE = 4;        // Size of a word in bytes
const EXPANDED_KEY_SIZE = 176; // Size of the expanded key (11 round keys * 16 bytes)

// Galois Field (GF(2^8)) constants for MixColumns operation
const GF_MODULUS = 0x1b;    // Irreducible polynomial: x^8 + x^4 + x^3 + x + 1
const GF_HIGH_BIT = 0x80;   // Used for carry detection in GF multiplication

// AES S-box
export const SBOX = new Uint8Array([
    
]);

// AES inverse S-box
export const INV_SBOX = new Uint8Array([
    
]);

// AES round constants
export const RCON = new Uint8Array([
    0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
]);

/**
 * Performs the SubBytes operation on the state array.
 * Applies the S-box substitution to each byte in the state.
 * 
 * @param {Uint8Array} state - The state array to transform
 * @param {boolean} [inverse=false] - Whether to use the inverse S-box
 */
export function subBytes(state, inverse = false) {
    const sbox = inverse ? INV_SBOX : SBOX;
    for (let i = 0; i < STATE_SIZE; i++) {
        state[i] = sbox[state[i]];
    }
}

/**
 * Performs the ShiftRows operation on the state array.
 * Each row is cyclically shifted by a different offset.
 * 
 * @param {Uint8Array} state - The state array to transform
 * @param {boolean} [inverse=false] - Whether to shift right (inverse) instead of left
 */
export function shiftRows(state, inverse = false) {
    const temp = new Uint8Array(STATE_SIZE);
    const shifts = inverse ? [0, 3, 2, 1] : [0, 1, 2, 3];

    // Row 0: no shift
    temp[0] = state[0];
    temp[4] = state[4];
    temp[8] = state[8];
    temp[12] = state[12];

    // Row 1: shift by 1
    const row1Shift = shifts[1];
    temp[1] = state[(1 + 4 * row1Shift) % 16];
    temp[5] = state[(5 + 4 * row1Shift) % 16];
    temp[9] = state[(9 + 4 * row1Shift) % 16];
    temp[13] = state[(13 + 4 * row1Shift) % 16];

    // Row 2: shift by 2
    const row2Shift = shifts[2];
    temp[2] = state[(2 + 4 * row2Shift) % 16];
    temp[6] = state[(6 + 4 * row2Shift) % 16];
    temp[10] = state[(10 + 4 * row2Shift) % 16];
    temp[14] = state[(14 + 4 * row2Shift) % 16];

    // Row 3: shift by 3
    const row3Shift = shifts[3];
    temp[3] = state[(3 + 4 * row3Shift) % 16];
    temp[7] = state[(7 + 4 * row3Shift) % 16];
    temp[11] = state[(11 + 4 * row3Shift) % 16];
    temp[15] = state[(15 + 4 * row3Shift) % 16];

    state.set(temp);
}

/**
 * Specialized multiplication functions for GF(2^8) with AES field polynomial.
 * These are optimized for the specific constant values used in MixColumns.
 */


function gmul2(a) {
    const highBit = (a & GF_HIGH_BIT) !== 0;
    a = (a << 1) & 0xff;
    if (highBit) a ^= GF_MODULUS;
    return a;
}

function gmul3(a) {
    return gmul2(a) ^ a;
}

function gmul9(a) {
    return gmul2(gmul2(gmul2(a))) ^ a;
}

function gmul11(a) {
    return gmul2(gmul2(gmul2(a))) ^ gmul2(a) ^ a;
}

function gmul13(a) {
    return gmul2(gmul2(gmul2(a))) ^ gmul2(gmul2(a)) ^ a;
}

function gmul14(a) {
    return gmul2(gmul2(gmul2(a))) ^ gmul2(gmul2(a)) ^ gmul2(a);
}

/**
 * Performs the MixColumns operation on the state array.
 * Each column is treated as a polynomial over GF(2^8) and multiplied
 * by a fixed polynomial a(x) = {03}x^3 + {01}x^2 + {01}x + {02}.
 * 
 * @param {Uint8Array} state - The state array to transform
 * @param {boolean} [inverse=false] - Whether to use inverse coefficients
 */
export function mixColumns(state, inverse = false) {
    const temp = new Uint8Array(STATE_SIZE);

    if (inverse) {
        // Inverse MixColumns with coefficients [0x0e, 0x0b, 0x0d, 0x09]
        for (let i = 0; i < 4; i++) {
            const col = i * 4;
            const s0 = state[col];
            const s1 = state[col + 1];
            const s2 = state[col + 2];
            const s3 = state[col + 3];

            temp[col] = gmul14(s0) ^ gmul11(s1) ^ gmul13(s2) ^ gmul9(s3);
            temp[col + 1] = gmul9(s0) ^ gmul14(s1) ^ gmul11(s2) ^ gmul13(s3);
            temp[col + 2] = gmul13(s0) ^ gmul9(s1) ^ gmul14(s2) ^ gmul11(s3);
            temp[col + 3] = gmul11(s0) ^ gmul13(s1) ^ gmul9(s2) ^ gmul14(s3);
        }
    } else {
        // Forward MixColumns with coefficients [0x02, 0x03, 0x01, 0x01]
        for (let i = 0; i < 4; i++) {
            const col = i * 4;
            const s0 = state[col];
            const s1 = state[col + 1];
            const s2 = state[col + 2];
            const s3 = state[col + 3];

            temp[col] = gmul2(s0) ^ gmul3(s1) ^ s2 ^ s3;
            temp[col + 1] = s0 ^ gmul2(s1) ^ gmul3(s2) ^ s3;
            temp[col + 2] = s0 ^ s1 ^ gmul2(s2) ^ gmul3(s3);
            temp[col + 3] = gmul3(s0) ^ s1 ^ s2 ^ gmul2(s3);
        }
    }
    state.set(temp);
}

/**
 * Expands a 16-byte key into round keys for AES encryption/decryption.
 * Implements the AES key schedule algorithm.
 * 
 * @param {Uint8Array} key - The 16-byte key to expand
 * @returns {Uint8Array} The expanded key (176 bytes for AES-128)
 */
export function expandKey(key) {
    const expandedKey = new Uint8Array(EXPANDED_KEY_SIZE);
    expandedKey.set(key);

    let rconIndex = 0;
    for (let i = STATE_SIZE; i < EXPANDED_KEY_SIZE; i += WORD_SIZE) {
        // Copy previous word
        const temp = expandedKey.slice(i - WORD_SIZE, i);

        // Key schedule core for first word of each round
        if (i % STATE_SIZE === 0) {
            // Rotate word
            const t = temp[0];
            temp.copyWithin(0, 1);
            temp[3] = t;

            // Apply S-box and XOR with round constant
            temp.forEach((byte, j) => {
                temp[j] = SBOX[byte];
            });
            temp[0] ^= RCON[rconIndex++];
        }

        // XOR with word 4 positions back
        for (let j = 0; j < WORD_SIZE; j++) {
            expandedKey[i + j] = expandedKey[i - STATE_SIZE + j] ^ temp[j];
        }
    }

    return expandedKey;
} 